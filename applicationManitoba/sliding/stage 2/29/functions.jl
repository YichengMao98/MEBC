...
function rstar_block_update!(R::Vector{Int}, Istar::Vector{Int}, N::Int, S0::Int, I0::Int;
                             ...
                             nUpdates::Int=200)
    τ = length(R)
    ll_cur = loglik_aug(Istar, R, N, S0, I0; beta=beta, alpha=alpha, gamma=gamma, psi=psi)
    @inbounds for _ in 1:nUpdates
        ...
        ll_prop = loglik_aug(Istar, Rprop, N, S0, I0; beta=beta, alpha=alpha, gamma=gamma, psi=psi)
...
# --- robust covariance to ensure SPD ---
...
function mcmc_one_chain_with_Rstar!(Istar_obs::Vector{Int}, N::Int, S0::Int, I0::Int;
    ...
    elseif fit_mech === :sliding
        isnothing(k_max_fixed) && error("For :sliding, pass k_max_fixed")
    ...
    ll = loglik_aug(Istar_obs, R, N, S0, I0;
                    beta=theta_cont[1], alpha=theta_cont[2], gamma=theta_cont[3], psi=psi)
    ...
    initial_stds = d_cont == 3 ? [0.005, 0.00005, 0.005] :
                                  [0.005, 0.00005, 0.005, 0.005]
    ...
    for i in 1:n_iter
        ...
        # proposal distribution (adaptive mixture during adaptation; fixed after)
        prop_dist = nothing
        if in_adaptation_phase
        ...
        if i % 1000 == 0
            ...
            if d_cont == 4
                @info(@sprintf("[%s] iter %d/%d elapsed=%.1fs, rate=%.3f, mean=[%.3f, %.5f, %.3f, %.3f], std=[%.4f, %.6f, %.4f, %.4f]%s",
                               ...
                               online_cov.n > 0 ? online_cov.mean[3] : theta_cont[3],
                               online_cov.n > 0 ? online_cov.mean[4] : (d_cont==4 ? theta_cont[4] : NaN),
                               prop_std[1], prop_std[2], prop_std[3], prop_std[4], phase_info))
            else
                @info(@sprintf("[%s] iter %d/%d elapsed=%.1fs, rate=%.3f, mean=[%.3f, %.5f, %.3f], std=[%.4f, %.6f, %.4f]%s",
                               String(fit_mech), i, n_iter, el, accept_rate,
                               online_cov.n > 0 ? online_cov.mean[1] : theta_cont[1],
                               online_cov.n > 0 ? online_cov.mean[2] : theta_cont[2],
                               online_cov.n > 0 ? online_cov.mean[3] : theta_cont[3],
...
function initθ_for_chain(tag::Symbol)
    init_beta  = rand(Uniform(0.8, 1.2))
    init_alpha = rand(Uniform(0.002, 0.004))
