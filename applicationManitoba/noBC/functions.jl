using Dates
using Printf
using Distributions
using LinearAlgebra
using DelimitedFiles

# ----------------- constants and helpers -----------------
clamp01(x) = x < 0 ? 0.0 : (x > 1 ? 1.0 : x)

# pSI = clamp01(1 - exp(-beta) * (Iinf / N))
function loglik_aug(Istar::Vector{Int}, Rstar::Vector{Int}, N::Int, S0::Int, I0::Int;
                    beta::Float64, gamma::Float64, return_vec::Bool=false)
    τ = length(Istar)
    (beta <= 0 || gamma <= 0) && return return_vec ? vcat(fill(-Inf, τ)) : -Inf
    length(Rstar) == τ || error("length(Rstar)!=τ")
    (S0 < 0 || I0 < 0 || S0 + I0 > N) && return return_vec ? vcat(fill(-Inf, τ)) : -Inf

    S = S0
    Iinf = I0
    pIR = clamp01(1 - exp(-gamma))

    if return_vec
        ll_vec = Vector{Float64}(undef, τ)
        @inbounds for t in 1:τ
            (Istar[t] < 0 || Istar[t] > S) && return vcat(fill(-Inf, τ))
            pSI = clamp01(1 - exp(-beta) * (Iinf / N))
            l1 = logpdf(Binomial(S, pSI), Istar[t])
            (Rstar[t] < 0 || Rstar[t] > Iinf) && return vcat(fill(-Inf, τ))
            l2 = logpdf(Binomial(Iinf, pIR), Rstar[t])
            ll_vec[t] = l1 + l2
            S  -= Istar[t]
            Iinf += Istar[t] - Rstar[t]
            (S < 0 || Iinf < 0) && return vcat(fill(-Inf, τ))
        end
        return ll_vec
    else
        ll_sum = 0.0
        @inbounds for t in 1:τ
            (Istar[t] < 0 || Istar[t] > S) && return -Inf
            pSI = clamp01(1 - exp(-beta) * (Iinf / N))
            l1 = logpdf(Binomial(S, pSI), Istar[t])
            (Rstar[t] < 0 || Rstar[t] > Iinf) && return -Inf
            l2 = logpdf(Binomial(Iinf, pIR), Rstar[t])
            ll_sum += (l1 + l2)
            S  -= Istar[t]
            Iinf += Istar[t] - Rstar[t]
            (S < 0 || Iinf < 0) && return -Inf
        end
        return ll_sum
    end
end

function rstar_block_update!(R::Vector{Int}, Istar::Vector{Int}, N::Int, S0::Int, I0::Int;
                             beta::Float64, gamma::Float64,
                             nUpdates::Int=200)
    τ = length(R)
    ll_cur = loglik_aug(Istar, R, N, S0, I0; beta=beta, gamma=gamma)
    @inbounds for _ in 1:nUpdates
        moveType = rand(1:3)
        Rprop = copy(R)
        g = 0.0
        if moveType == 1
            addIdx = rand(1:τ)
            Rprop[addIdx] += 1
            newPossibleSubtract = count(>(0), Rprop)
            g = -log(newPossibleSubtract) + log(τ)
        elseif moveType == 2
            possibleSubtract = findall(>(0), R)
            isempty(possibleSubtract) && continue
            subtractIdx = rand(possibleSubtract)
            addIdx = rand(1:τ)
            Rprop[subtractIdx] -= 1
            Rprop[addIdx]      += 1
            newPossibleSubtract = count(>(0), Rprop)
            g = -log(newPossibleSubtract) + log(length(possibleSubtract))
        else
            possibleSubtract = findall(>(0), R)
            isempty(possibleSubtract) && continue
            subtractIdx = rand(possibleSubtract)
            Rprop[subtractIdx] -= 1
            g = -log(τ) + log(length(possibleSubtract))
        end
        ll_prop = loglik_aug(Istar, Rprop, N, S0, I0; beta=beta, gamma=gamma)
        loga = (ll_prop - ll_cur) + g
        if log(rand()) < loga
            R .= Rprop
            ll_cur = ll_prop
        end
    end
    return R, ll_cur
end

function logprior_cont(θ::Vector{Float64})
    (θ[1] <= 0 || θ[2] <= 0) && return -Inf
    lp = logpdf(Uniform(0,100), θ[1]) +    # beta ~ Uniform(0,100)
         logpdf(Gamma(aa, bb), θ[2])       # gamma ~ Gamma(aa, bb)  (shape, scale)
    return lp
end

mutable struct OnlineCovariance{T<:AbstractFloat}
    n::Int
    mean::Vector{T}
    cov::Matrix{T}
    d::Int
end

function OnlineCovariance(d::Int)
    OnlineCovariance(0, zeros(d), zeros(d, d), d)
end

function update_online_cov!(oc::OnlineCovariance, new_sample::Vector{<:Number})
    oc.n += 1
    if oc.n == 1
        oc.mean = collect(Float64.(new_sample))
    else
        old_mean = oc.mean
        oc.mean = old_mean + (new_sample .- old_mean) ./ oc.n
        oc.cov = oc.cov + (new_sample .- old_mean) * (new_sample .- oc.mean)'
    end
    return
end

function get_diagonal_variances(online_cov::OnlineCovariance)
    if online_cov.n > 1
        cov_matrix = online_cov.cov ./ (online_cov.n - 1)
        return diag(cov_matrix)
    else
        return zeros(online_cov.d)
    end
end

function robust_proposal_cov(oc::OnlineCovariance; shrink=0.1, jitter0=1e-8, max_backoff=8)
    oc.n > 1 || return nothing
    C = oc.cov ./ (oc.n - 1)
    C = 0.5 .* (C .+ C')
    C = Symmetric((1 - shrink) .* C .+ shrink .* Diagonal(max.(diag(C), eps())))
    λ = jitter0
    for _ in 1:max_backoff
        try
            cholesky(C + λ*I, check=true)
            return C + λ*I
        catch
            λ *= 10
        end
    end
    return Symmetric(Diagonal(max.(diag(C), λ)))
end

# ----------------- main MCMC (no alarm/memory) -----------------
function mcmc_one_chain_with_Rstar!(Istar_obs::Vector{Int}, N::Int, S0::Int, I0::Int;
                                    n_iter::Int=1_000_000,
                                    initθ::Vector{Float64})
    tau = length(Istar_obs)
    (S0 < 0 || I0 < 0 || S0 + I0 > N) && error("Invalid initial S0/I0 relative to N")

    d_cont = 2
    theta_cont = copy(initθ[1:d_cont])

    gamma0 = theta_cont[2]
    pIR0 = clamp(1 - exp(-gamma0), 0, 1)

    R = zeros(Int, tau)
    S = S0
    Iinf = I0
    for t in 1:tau
        R[t] = min(Iinf, round(Int, Iinf * pIR0))
        (Istar_obs[t] > S) && error("Istar exceeds available susceptibles at t=$t")
        S -= Istar_obs[t]
        Iinf += Istar_obs[t] - R[t]
        (S < 0 || Iinf < 0) && error("Bad initial Rstar; change init.")
    end

    ll = loglik_aug(Istar_obs, R, N, S0, I0; beta=theta_cont[1], gamma=theta_cont[2])
    lp = logprior_cont(theta_cont)

    samples = Array{Float32}(undef, n_iter, d_cont)

    burnin = n_iter ÷ 2
    thin = 10
    n_saved_loglik = max(0, (n_iter - burnin) ÷ thin)
    loglik_aug_vecs = Array{Float32}(undef, n_saved_loglik, tau)
    loglik_save_counter = 0

    initial_stds = [0.005, 0.005]
    Sigma_initial = Diagonal(initial_stds.^2) / 2

    online_cov = OnlineCovariance(d_cont)
    adaptation_cutoff = n_iter ÷ 2
    prop_dist_fixed = nothing
    prop_std_fixed = nothing

    accept_count = 0
    t0 = now()

    for i in 1:n_iter
        R, ll = rstar_block_update!(R, Istar_obs, N, S0, I0;
                                    beta=theta_cont[1], gamma=theta_cont[2],
                                    nUpdates=200)

        in_adaptation_phase = i <= adaptation_cutoff
        if in_adaptation_phase
            update_online_cov!(online_cov, theta_cont)
        end

        prop_dist = nothing
        if in_adaptation_phase
            Sigma_adaptive = robust_proposal_cov(online_cov)
            if Sigma_adaptive === nothing
                prop_dist = MvNormal(zeros(online_cov.d), Sigma_initial)
            else
                prop_dist = MixtureModel([MvNormal(zeros(online_cov.d), Sigma_adaptive),
                                          MvNormal(zeros(online_cov.d), Sigma_initial)],
                                         [0.8, 0.2])
            end

            if i == adaptation_cutoff
                prop_dist_fixed = prop_dist
                Sigma_adaptive_cut = robust_proposal_cov(online_cov)
                if Sigma_adaptive_cut === nothing
                    prop_std_fixed = sqrt.(diag(Sigma_initial))
                else
                    actual_sigma = 0.8 .* Sigma_adaptive_cut .+ 0.2 .* Sigma_initial
                    prop_std_fixed = sqrt.(diag(actual_sigma))
                end
            end
        else
            prop_dist = prop_dist_fixed !== nothing ? prop_dist_fixed :
                        MvNormal(zeros(d_cont), Sigma_initial)
        end

        theta_prop = theta_cont .+ rand(prop_dist)

        if all(theta_prop .> 0)
            lp_prop = logprior_cont(theta_prop)
            if lp_prop !== -Inf
                ll_prop = loglik_aug(Istar_obs, R, N, S0, I0;
                                     beta=theta_prop[1], gamma=theta_prop[2])

                if log(rand()) < (ll_prop + lp_prop - ll - lp)
                    theta_cont .= theta_prop
                    ll = ll_prop
                    lp = lp_prop
                    accept_count += 1
                end
            end
        end

        samples[i, :] = Float32.(theta_cont)

        if i > burnin && (i - burnin) % thin == 0
            loglik_save_counter += 1
            loglik_aug_vecs[loglik_save_counter, :] =
                Float32.(loglik_aug(Istar_obs, R, N, S0, I0;
                                    beta=theta_cont[1], gamma=theta_cont[2],
                                    return_vec=true))
        end

        if i % 1000 == 0
            el = Dates.value(now() - t0) / 1000
            accept_rate = accept_count / i

            if in_adaptation_phase
                Sigma_adaptive = robust_proposal_cov(online_cov)
                if Sigma_adaptive === nothing
                    prop_std = sqrt.(diag(Sigma_initial))
                else
                    actual_sigma = 0.8 .* Sigma_adaptive .+ 0.2 .* Sigma_initial
                    prop_std = sqrt.(diag(actual_sigma))
                end
            else
                prop_std = prop_std_fixed !== nothing ? prop_std_fixed : sqrt.(diag(Sigma_initial))
            end

            @info(@sprintf("[no-alarm] iter %d/%d elapsed=%.1fs, rate=%.3f, mean=[%.3f, %.3f], std=[%.4f, %.4f]%s",
                           i, n_iter, el, accept_rate,
                           online_cov.n > 0 ? online_cov.mean[1] : theta_cont[1],
                           online_cov.n > 0 ? online_cov.mean[2] : theta_cont[2],
                           prop_std[1], prop_std[2],
                           in_adaptation_phase ? " [ADAPT]" : " [FIXED]"))
        end
    end

    return samples, loglik_aug_vecs
end

# ----------------- init theta -----------------
function initθ_for_chain(tag::Symbol)
    init_beta  = rand(Uniform(1, 1.6))
    init_gamma = rand(Gamma(aa, bb))
    return [init_beta, init_gamma]
end

# ----------------- diagnostics & summaries -----------------
# Gelman-Rubin R-hat
function rhat_gelman_rubin(chains_2d::AbstractMatrix)
    m, n = size(chains_2d)
    chain_means = vec(mean(chains_2d, dims=2))
    chain_vars  = vec(var(chains_2d, dims=2, corrected=true))
    W = mean(chain_vars)
    if !isfinite(W) || W <= 0
        return 1.0
    end
    B = n * var(chain_means, corrected=true)
    var_hat = ((n - 1) / n) * W + (1.0 / n) * B
    var_hat = max(var_hat, 1e-16)
    Rhat = sqrt(var_hat / W)
    return max(Rhat, 1.0)
end

function compute_waic(all_loglik_aug_vecs)
    combined_logliks = vcat(all_loglik_aug_vecs...)
    if any(isinf, combined_logliks)
        @warn "Encountered infinite log-likelihoods. WAIC will be -Inf."
        return -Inf
    end
    lppd = sum(log.(mean(exp.(combined_logliks), dims=1)))
    pwaic = sum(var(combined_logliks, dims=1))
    waic = -2 * lppd + 2 * pwaic
    return waic
end

function summarize_posterior(samples::AbstractMatrix; burn_in::Int=0, names::Vector{String}=["beta","gamma"])
    samples_burned = burn_in > 0 ? samples[burn_in+1:end, :] : samples
    summary_dict = Dict{String, Any}()
    for (i, p_name) in enumerate(names)
        param_samples = Float64.(samples_burned[:, i])
        med = median(param_samples)
        ci  = quantile(param_samples, [0.025, 0.975])
        summary_dict[p_name] = Dict("median" => med, "95%CI_low" => ci[1], "95%CI_high" => ci[2])
    end
    return summary_dict
end

# HDPI helper
function get_hdpi(x, alpha=0.95)
    sorted_x = sort(x)
    n = length(sorted_x)
    n_hdi = floor(Int, alpha * n)
    if n_hdi == 0
        return [sorted_x[1], sorted_x[end]]
    end
    range_hdi = sorted_x[n_hdi+1:end] - sorted_x[1:n-n_hdi]
    idx = argmin(range_hdi)
    return [sorted_x[idx], sorted_x[idx+n_hdi]]
end

# ----------------- CSV writers -----------------
function write_csv(path::String, header::Vector{String}, M::AbstractMatrix)
    open(path, "w") do io
        println(io, join(header, ","))
        DelimitedFiles.writedlm(io, M, ',')
    end
end

function write_summary_csv(path::String, summary_data)
    open(path, "w") do io
        println(io, "Dataset,Parameter,Median,CI_Low,CI_High")
        for (dataset_idx, summary_dict) in enumerate(summary_data)
            for (param, values) in summary_dict
                println(io, "$dataset_idx,$param,$(values["median"]),$(values["95%CI_low"]),$(values["95%CI_high"])")
            end
        end
    end
end

function write_waic_csv(path::String, waic_data)
    open(path, "w") do io
        println(io, "Dataset,WAIC")
        for (idx, waic_val) in enumerate(waic_data)
            println(io, "$idx,$waic_val")
        end
    end
end

function write_gelman_rubin_csv(path::String, gr_data)
    open(path, "w") do io
        println(io, "Dataset,Parameter,GR_Value")
        for (dataset_idx, gr_dict) in enumerate(gr_data)
            for (param, gr_val) in gr_dict
                println(io, "$dataset_idx,$param,$gr_val")
            end
        end
    end
end

function write_simulation_stats(path::String, data::Vector{Matrix{Float64}})
    header = ["Day", "median", "mean", "hdi_low", "hdi_high"]
    all_datasets_matrix = hcat(data...)
    avg_matrix = mean(all_datasets_matrix, dims=2)
    output_matrix = hcat(1:size(data[1], 1), avg_matrix)
    write_csv(path, header, output_matrix)
end
